...
export const SHUIHU_PROPS: PropPlacement[] = [
  ...
  // 山石：贴幕落地（同枯树一个地面）——离灯放影子会掉出幕下缘、抬高又像悬空，
  // 贴幕（z→0）时影子基部正好落在幕下缘地面上，与枯树同地平线
  { name: 'shanshi', x: -0.3, z: 0.06, height: 0.45 },
];
...
export const XIYOU_PROPS: PropPlacement[] = [
  ...
  // 山石复用：贴幕落地（同水浒——贴幕影子基部才落在幕下缘地面上），幕左侧景
  { name: 'shanshi', x: 0.33, z: 0.06, height: 0.4 },
];
