...
本项目由 AI Agent 依据一份完整的构建指令文档生成：[KIMI-CODE-赛博皮影戏-构建指令.md](./KIMI-CODE-赛博皮影戏-构建指令.md)。这是一份端到端的构建规格 + 避坑地图（技术栈选型、资产管线提示词、投影实现、手势控制架构、踩坑清单），全部来自这个已跑通的实现。如果你想构建一个类似的项目，可以**直接把这份文档发给你的 Agent**（Kimi Code、Claude Code、Cursor 等），让它照着做。
...
| 按 `b` | 幕后模式：侧后 45° 机位环绕看灯/幕/影人/三根操纵杆 + 投影原理标注；左键拖拽环绕、滚轮缩放，再按 `b` 回前台（标定中让位） |
| 按 `r` | 重开当前对局 |
...
src/
  ...
  stage/   theater 舞台 / projection 彩色透光投影 / fire 三昧真火 / tree 枯树
           staff 哨棒 / goldenstaff 金箍棒 / spear 火尖枪 / cloud 筋斗云 / props 传统布景
...
完整规格与避坑地图见 [KIMI-CODE-赛博皮影戏-构建指令.md](./KIMI-CODE-赛博皮影戏-构建指令.md)。
